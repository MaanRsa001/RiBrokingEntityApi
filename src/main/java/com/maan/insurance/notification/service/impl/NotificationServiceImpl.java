...
import com.maan.insurance.notification.repo.NotifTemplateMasterRepository;
	...
	*/
	...
	public SuccessRes  SendMail(MailTriggerReq req ) {
		...
		try {
			...
			if(mailTo!=null && mailTo.size()>0 ) {
				...
				res.setMessage("Mail successfully sent");
			...
			mex.printStackTrace();
			res.setMessage("Mail Sent Failed");
		}	
		return res;
	}
	...
	public SuccessRes replayMail(ReplayMailTriggerReq req) {
		...
		try {
			...
			res.setMessage("message replied successfully ....");
			...
			mex.printStackTrace();
			res.setMessage("Replied Message Failed....");
		}
		return res;
	}
	@Override
