...
public class PropPremiumCustomRepositoryImpl implements PropPremiumCustomRepository {
	...
	public String getAccPeriod(String proposalNo) {
		...
		cq.select(root.get("rskAccountingPeriod").as(String.class).alias("RSK_ACCOUNTING_PERIOD")).where(
				cb.equal(root.get("rskProposalNumber"), proposalNo), cb.equal(root.get("rskEndorsementNo"), endoSq));
...
//root.get("rownum").alias("sno"),
	...
	@Override
	public List<Tuple> getPremiumFullList(PremiumListReq req) {
		...
		if("S".equalsIgnoreCase(req.getSearchType())){
				if(StringUtils.isNotBlank(req.getCompanyNameSearch())){
					n0 =cb.like(cb.upper(personalRoot.get("companyName").as(String.class)),"%"+req.getCompanyNameSearch().toUpperCase()+"%");
				}
				if(StringUtils.isNotBlank(req.getBrokerNameSearch())){
					n0 =cb.like(cb.upper(nameExpression.as(String.class)),"%"+req.getBrokerNameSearch().toUpperCase()+"%");
				}
				if(StringUtils.isNotBlank(req.getContractNoSearch())){
					n0 =cb.like(cb.upper(traRoot.get("contractNo").as(String.class)),"%"+req.getContractNoSearch().toUpperCase()+"%");
				}
				if(StringUtils.isNotBlank(req.getTransactionNoSearch())){
					n0 =cb.like(cb.upper(traRoot.get("transactionNo").as(String.class)),"%"+req.getTransactionNoSearch().toUpperCase()+"%");
				}
	...
	@Override
	public List<Tuple> getPremiumTempList(PremiumListReq req) {
		List<Tuple>list=null;
		CriteriaBuilder cb = em.getCriteriaBuilder();
		CriteriaQuery<Tuple> cq = cb.createTupleQuery();
		Root<TtrnRiskDetails> rkRoot = cq.from(TtrnRiskDetails.class);
	...
	@Override
	public List<Tuple> getPremiumRiFullList(PremiumListReq req) {
		List<Tuple>list=null;
		CriteriaBuilder cb = em.getCriteriaBuilder();
		CriteriaQuery<Tuple> cq = cb.createTupleQuery();
		Root<TtrnRiskDetails> rkRoot = cq.from(TtrnRiskDetails.class);
		Root<PersonalInfo> personalRoot = cq.from(PersonalInfo.class);
		Root<PersonalInfo> piRoot = cq.from(PersonalInfo.class);
		Root<RskPremiumDetailsRi> traRoot = cq.from(RskPremiumDetailsRi.class);
		Root<PositionMaster> pmroot = cq.from(PositionMaster.class);
		...
		cq.multiselect(
			   ...
			   traRoot.get("settlementStatus").alias("SETTLEMENT_STATUS"),
			   traRoot.get("transactionMonthYear").alias("TRANSACTION_DATE"),
			   traRoot.get("ritransactionNo").alias("RI_TRANSCATION_NO"),
			   traRoot.get("reverseTransactionNo").alias("REVERSE_TRANSACTION_NO"));
